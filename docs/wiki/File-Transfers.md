...
The checked-in driver validates p2prpc ownership and protocol integration, not Iroh throughput. Its eligibility flag covers the exact count/profile and required diagnostics, but the current driver does not automate every required fault or mixed-load measurement. A local, loopback, mocked, reduced, reconnecting, or diagnostics-incomplete run is not production evidence. GitHub Packages publication is independently gated by exact-commit public CI and therefore must not be presented as proof that the protected external evidence or separate two-host discovery/relay matrix in [Production Validation](Production-Validation.md) exists; this repository does not claim that evidence has already been obtained.
